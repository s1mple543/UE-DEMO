...
void AShooterNPC::BeginPlay()
{
	...
	// Only server-side NPCs should spawn weapons
	if (!HasAuthority())
	{
		return;
	}
	...
	// Disable FP mesh anim for NPCs to avoid ABP_FP_Weapon crash
	if (Weapon && Weapon->GetFirstPersonMesh())
	{
		Weapon->GetFirstPersonMesh()->SetAnimationMode(EAnimationMode::AnimationSingleNode);
		Weapon->GetFirstPersonMesh()->SetAnimation(nullptr);
	}
}
...
float AShooterNPC::TakeDamage(float Damage, struct FDamageEvent const& DamageEvent, AController* EventInstigator, AActor* DamageCauser)
{
	...
	// Save the last damage instigator for scoring when we die
	// Only count player-controlled instigators to prevent phantom kills
	if (EventInstigator && EventInstigator->GetPawn() && EventInstigator->GetPawn()->IsPlayerControlled())
	{
...
{
	// NPCs only need third-person animations. First-person weapon anim BPs
	// (e.g. ABP_FP_Weapon) call GetController and crash on AIControllers.
	if (IsValid(InWeapon))
	{
...
void AShooterNPC::Die()
{
	...
	// NOTE: OnEnemyKilled is NOT called here — it's triggered by OnPawnDeath.Broadcast() above
	// via the dynamic delegate bound in SpawnWaveEnemies/SpawnNextEnemy
	if (AShooterGameMode* GM = Cast<AShooterGameMode>(GetWorld()->GetAuthGameMode()))
	{
		GM->IncrementTeamScore(TeamByte);
	}

	// Give kill credit to the last damage instigator
	if (LastDamageInstigator)
	{
		if (AMultiShooterPlayerState* PS = LastDamageInstigator->GetPlayerState<AMultiShooterPlayerState>())
		{
			PS->AddKill();
		}
	}
