...
void AShooterWeapon::BeginPlay()
{
	...
	// If the owner is NOT a player, disable the FP mesh animation entirely.
	// ABP_FP_Weapon calls GetController() which crashes on AIController.
	if (PawnOwner && !PawnOwner->IsPlayerControlled())
	{
		if (FirstPersonMesh)
		{
			FirstPersonMesh->SetAnimationMode(EAnimationMode::AnimationSingleNode);
			FirstPersonMesh->SetAnimation(nullptr);
		}
	}
}
...
{
	// can't reload if we're already reloading or magazine is full
	...
	// Use a dedicated reload timer (separate from RefireTimer to avoid conflicts)
...
void AShooterWeapon::FireProjectile(const FVector& TargetLocation)
{
	...
	// update the weapon HUD (must always be called even at 0 so UI reflects empty)
	WeaponOwner->UpdateWeaponHUD(CurrentBullets, MagazineSize);

	// if the clip is depleted, stop firing
	if (CurrentBullets <= 0)
	{
		StopFiring();
	}
